...
var Toast = (function() {
    return {
        ...
        processQueue : function(toast){
            var $toast = $('.' + Toast.DEFAULT_CLASS);
    
            if ($toast.length > 0)
                $toast.remove();
        
            var cssClass = Toast.DEFAULT_CLASS + (toast.class != '' ? ' ' + toast.class : '');
            var style    = "position:absolute; display:block; text-align:center; height: auto; overflow: hidden; padding:10px 18px 10px 18px; opacity:0; z-index:100000;";
                style   += "border-radius: " + toast.radius + "px; -webkit-border-radius: " + toast.radius + "px; -ms-border-radius: \
                           " + toast.radius + "px; -o-border-radius:" + toast.radius + "px; -moz-border-radius: " + toast.radius + "px;";
                style   += "-webkit-transition: opacity " + toast.appearTime + "s; -o-transition: opacity " + toast.appearTime + "s; transition: opacity \
                           " + toast.appearTime + "s; -moz-transition: opacity " + toast.appearTime + "s; -ms-transition: opacity " + toast.appearTime + "s;";
            ...
            setTimeout(function(){
                ...
                switch(toast.position){
                    case Toast.POSITION_TOP     : $toast.css('top', toast.top); break;
                    case Toast.POSITION_MIDDLE  : $toast.css('top', ($(window).height() - $toast.outerHeight(true)) / 2); break;
                    default                     : $toast.css('bottom', toast.top);  break;
                }

                switch(toast.align){
                    case Toast.ALIGN_LEFT  : $toast.css('left', toast.left); break;
                    case Toast.ALIGN_RIGHT : $toast.css('right', toast.left); break;
                    default                : $toast.css('left', ($(window).width() - $toast.outerWidth(true)) / 2); break;
                }
